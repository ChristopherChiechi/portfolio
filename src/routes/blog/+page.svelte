<script lang="ts">
	...
	let filters: Array<SkillFilter> = $state(
		SkillsData.items.filter((it) => {
			return BlogData.items.some((blog) =>
	...
	let result = $derived(
		BlogData.items.filter((blog) => {
			...
			const isSearched =
				...
				blog.shortDescription.trim().toLowerCase().includes(search.trim().toLowerCase()) ||
				blog.type.trim().toLowerCase().includes(search.trim().toLowerCase());
